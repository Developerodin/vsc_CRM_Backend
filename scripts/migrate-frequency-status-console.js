...
// We'll import the Timeline model dynamically
...
const migrateFrequencyStatus = async () => {
  try {
    console.log('Starting frequency status migration...');
    ...
    console.log(`Found ${timelines.length} timelines to migrate`);
    ...
    for (const timeline of timelines) {
      try {
        ...
        migratedCount++;
        console.log(`Migrated timeline ${timeline._id} with ${frequencyStatus.length} frequency periods`);
        
      } catch (error) {
        errorCount++;
        console.error(`Error migrating timeline ${timeline._id}: ${error.message}`);
      }
    }
    
    console.log(`Migration completed!`);
    console.log(`- Migrated: ${migratedCount} timelines`);
